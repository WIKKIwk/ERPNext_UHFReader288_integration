...
async function api(path, body) {
  ...
  return parseMaybeJson(data.result ?? data);
}
...
let selectedEpc = '';
let rwPickCache = '';
let rwPickFocused = false;
let rwPickNeedsRefresh = false;
let powerChangeCooldownUntil = 0;
let invStartedAt = 0;
let speedMarks = [];
...
function renderTags() {
  ...
  for (const tr of tbody.querySelectorAll('tr')) {
    ...
    const epcCell = tr.querySelector('.col-epc');
    if (epcCell) {
      epcCell.addEventListener('click', (ev) => {
        ev.stopPropagation();
        promptEditEpc(epc);
      });
    }
    tr.addEventListener('click', () => {
      syncRwSimpleFromEpc(epc, { prefillNew: true, focusNew: true });
      renderTags();
      setView('c1g2', 'rw');
...
function clampPowerValue(raw, { min = 0, max = 33 } = {}) {
  const n = Number(raw);
  if (!Number.isFinite(n)) return null;
  return Math.max(min, Math.min(max, Math.trunc(n)));
}

function ensureAntPowerCache(count) {
  const fallback = clampPowerValue($('power')?.value ?? 30) ?? 30;
  const next = [];
  for (let i = 0; i < count; i += 1) {
    const v = antPowerCache[i];
    next[i] = Number.isFinite(v) ? v : fallback;
  }
  antPowerCache = next;
  return next;
}

function applyAntPowerValues(values, { quiet = false } = {}) {
  ...
  for (let i = 0; i < count; i += 1) {
    const raw = values[i];
    const v = clampPowerValue(raw, { min: 0, max: 33 });
    next[i] = v === null ? fallback : v;
  }
  antPowerCache = next;
  for (let i = 1; i <= count; i += 1) {
    const el = $(`antPower_${i}`);
    if (el) el.value = String(next[i - 1]);
  }
  if (!quiet) saveUiState();
}

function collectAntPowerValues() {
  const count = getAntennaCountFromUi();
  const values = [];
  for (let i = 1; i <= count; i += 1) {
    const el = $(`antPower_${i}`);
    if (!el) continue;
    const v = clampPowerValue(el.value, { min: 0, max: 33 });
    if (v === null) throw new Error(`ANT${i} quvvat noto‘g‘ri (0–33).`);
    el.value = String(v);
    values.push(v);
  }
  if (values.length) {
    antPowerCache = values.slice(0);
    saveUiState();
  }
  return values;
}

async function waitForPowerCooldown() {
  const now = Date.now();
  if (now >= powerChangeCooldownUntil) return;
  const waitMs = powerChangeCooldownUntil - now;
  if (waitMs <= 0) return;
  await new Promise((resolve) => setTimeout(resolve, waitMs));
}

function renderAntPowerGrid() {
  const wrap = $('antPowerGrid');
  if (!wrap) return;
  const count = getAntennaCountFromUi();
  const values = ensureAntPowerCache(count);
  const cols = count <= 4 ? count : 4;
  wrap.style.gridTemplateColumns = `repeat(${cols}, minmax(110px, 1fr))`;
  wrap.style.gridAutoFlow = 'row';
  wrap.style.gridAutoRows = 'minmax(0, auto)';
  wrap.innerHTML = '';
  for (let i = 1; i <= count; i += 1) {
    const cell = document.createElement('label');
    cell.className = 'ant-power-cell';
    const title = document.createElement('span');
    title.textContent = `ANT${i}`;
    const input = document.createElement('input');
    input.id = `antPower_${i}`;
    input.type = 'number';
    input.min = '0';
    input.max = '33';
    input.value = String(values[i - 1]);
    input.dataset.ant = String(i);
    input.addEventListener('change', () => {
      const next = clampPowerValue(input.value, { min: 0, max: 33 });
      if (next === null) {
        const prev = antPowerCache[i - 1];
        input.value = String(Number.isFinite(prev) ? prev : values[i - 1]);
        toast(`ANT${i} quvvat noto‘g‘ri (0–33).`, 'warn', { ttlMs: 5000 });
        return;
      }
      input.value = String(next);
      antPowerCache[i - 1] = next;
      saveUiState();
    });
...
function getBasicParams() {
  ...
  return {
    ...
    antPower: Array.isArray(antPowerCache) ? antPowerCache.slice(0) : [],
  };
}
...
function applyBasicState(state) {
  ...
  if (Array.isArray(state.antPower)) applyAntPowerValues(state.antPower, { quiet: true });
...
function renderRwPickList({ force = false } = {}) {
  const list = $('rwPickList');
  if (!list) return;
  const input = $('rwEpc');
  if (!force && (rwPickFocused || document.activeElement === input)) {
    rwPickNeedsRefresh = true;
    return;
  }
  const rows = [...tags.values()].sort((a, b) => (b.lastSeen ?? 0) - (a.lastSeen ?? 0));
  const uniq = [];
  const seen = new Set();
  for (const t of rows) {
    const epc = compactHex(t.epcId || '');
    if (!epc || seen.has(epc)) continue;
    seen.add(epc);
    uniq.push(epc);
  }
  const key = uniq.join('|');
  if (key === rwPickCache) return;
  rwPickCache = key;
  list.innerHTML = uniq.map((epc) => `<option value="${epc}"></option>`).join('');
}

function syncRwNewEpc({ sanitizeInput = false } = {}) {
  const newEl = $('rwNewEpc');
  if (!newEl) return;
  const compact = compactHex(newEl.value);
  if (sanitizeInput) newEl.value = compact;
  if ($('rwData')) $('rwData').value = compact;
  updateRwMeta();
}

function syncRwSimpleFromEpc(epc, { prefillNew = true, focusNew = false } = {}) {
  const clean = compactHex(epc);
  if (!clean) return;
  selectedEpc = clean;
  if ($('rwEpc')) $('rwEpc').value = clean;
  const newEl = $('rwNewEpc');
  if (newEl) {
    const cur = compactHex(newEl.value);
    if (prefillNew && !cur) newEl.value = clean;
  }
  if ($('rwMem')) $('rwMem').value = '1';
  if ($('rwWordPtr')) $('rwWordPtr').value = '2';
  if ($('rwNum')) $('rwNum').value = String(Math.max(1, Math.trunc(clean.length / 4)));
  syncRwNewEpc({ sanitizeInput: false });
  if (focusNew && newEl) {
    try {
      newEl.focus();
    } catch {
...
function promptEditEpc(epc) {
  const clean = compactHex(epc);
  if (!clean) return;
  const nextRaw = window.prompt('Yangi EPC (hex):', clean);
  if (nextRaw == null) return;
  const next = compactHex(nextRaw);
  if (!next) {
    toast('Yangi EPC bo‘sh.', 'warn');
    return;
  }
  syncRwSimpleFromEpc(clean, { prefillNew: false, focusNew: false });
  if ($('rwNewEpc')) $('rwNewEpc').value = next;
  syncRwNewEpc({ sanitizeInput: true });
  setView('c1g2', 'rw');
  const btn = $('btnRwEpcWrite');
  if (btn) btn.click();
}
...
function buildSimpleEpcWriteBody() {
  const epc = compactHex($('rwEpc')?.value);
  const data = compactHex($('rwNewEpc')?.value);
  const passwordRaw = compactHex($('rwPwd')?.value);
  const password = passwordRaw || '00000000';

  if (!epc) throw new Error('EPC tanlanmagan.');
  if (!data) throw new Error('Yangi EPC bo‘sh.');
  if (!isHexStrict(epc) || epc.length % 2 !== 0) throw new Error('EPC faqat hex bo‘lsin (uzunligi juft).');
  if (!isHexStrict(data) || data.length % 2 !== 0) throw new Error('Yangi EPC faqat hex bo‘lsin (uzunligi juft).');
  if (data.length % 4 !== 0) throw new Error('Yangi EPC uzunligi 4 ga karrali bo‘lishi kerak (1 word = 4 hex).');
  if (data.length !== epc.length) throw new Error('Yangi EPC uzunligi eski EPC bilan bir xil bo‘lishi kerak.');
  if (!isHexStrict(password) || password.length !== 8) throw new Error('Access parol 8 ta hex bo‘lishi kerak (masalan: 00000000).');

  return { epc, mem: 1, wordPtr: 2, password, data };
}
...
function normalizeRc(result) {
  const parsed = parseMaybeJson(result);
  if (parsed && typeof parsed === 'object' && 'rc' in parsed) return Number(parsed.rc);
  if (typeof parsed === 'number') return Number(parsed);
...
function describeReturnCode(rc) {
  switch (Number(rc)) {
    case 0x00:
    case 0x26:
      return 'success';
    ...
    case 0x0c:
      return 'Access Password Cannot be Zero';
    case 0x0d:
...
function formatWriteOutput(result, body) {
  ...
  const rc = normalizeRc(result);
  lines.push(`Bank: ${rwBankName(body.mem)} | Word Ptr: ${body.wordPtr} | Data: ${words} word (${bytes} byte)`);
  if (rc !== undefined) {
    const desc = describeReturnCode(rc);
    const ok = Number(rc) === 0;
    lines.push(`Natija: rc=${rc}${ok ? ' (OK)' : ''}${desc ? ` (${desc})` : ''}`);
  } else if (result !== undefined) lines.push(`Natija (raw): ${JSON.stringify(result)}`);
  lines.push(`Data: ${groupHex(body.data)}`);
  return lines.join('\n');
}

async function verifyEpcWrite(body) {
  const readBody = {
    epc: body.data,
    mem: body.mem,
    wordPtr: body.wordPtr,
    num: Math.max(1, Math.trunc(body.data.length / 4)),
    password: body.password,
  };
  try {
    const res = await api('/api/read', readBody);
    const raw = res?.data;
    const extracted = extractHexDigits(String(raw ?? ''));
    const hex = extracted ? extracted.toUpperCase() : '';
    const want = body.data.toUpperCase();
    return { ok: hex === want, raw, hex, want };
  } catch (e) {
...
function updateInvDigits() {
  ...
  const invActive = invDesired || currentStatus.inventoryStarted;
  const speed = invActive ? speedMarks.length / (windowMs / 1000) : 0;
  const runtime = invActive && invStartedAt ? now - invStartedAt : 0;
...
function applyStatus(st) {
  ...
  setLocked($('btnInvStart'), !currentStatus.connected || invDesired);
  setLocked($('btnInvStop'), !invDesired && (!currentStatus.connected || !invRunning));
...
function bind() {
  ...
  for (const id of ['rwMem', 'rwWordPtr', 'rwNum', 'rwPwd', 'rwData', 'rwAutoStopInv']) {
    $(id).addEventListener('input', () => {
      updateRwMeta();
    });
    $(id).addEventListener('change', () => {
      if (id === 'rwPwd' || id === 'rwData') {
  ...
  if ($('rwEpc')) {
    $('rwEpc').addEventListener('input', () => {
      syncRwSimpleFromEpc($('rwEpc').value, { prefillNew: false });
    });
    $('rwEpc').addEventListener('focus', () => {
      rwPickFocused = true;
    });
    $('rwEpc').addEventListener('blur', () => {
      rwPickFocused = false;
      if (rwPickNeedsRefresh) {
        rwPickNeedsRefresh = false;
        renderRwPickList({ force: true });
      }
    });
    $('rwEpc').addEventListener('change', () => {
      $('rwEpc').value = compactHex($('rwEpc').value);
      syncRwSimpleFromEpc($('rwEpc').value, { prefillNew: true });
      rwPickNeedsRefresh = false;
      saveUiState();
    });
  }
  ...
  if ($('rwNewEpc')) {
    $('rwNewEpc').addEventListener('input', () => {
      syncRwNewEpc({ sanitizeInput: false });
    });
    $('rwNewEpc').addEventListener('change', () => {
      syncRwNewEpc({ sanitizeInput: true });
      saveUiState();
    });
  }
  ...
  if ($('btnBridgeRestart')) {
    $('btnBridgeRestart').onclick = async () => {
      await runBusy($('btnBridgeRestart'), 'Restart…', async () => {
        try {
          await api('/api/bridge/restart', {});
          await refreshStatus();
          toast('Servis qayta ishga tushdi.', 'ok');
          logLine('Servis (Java bridge) qayta ishga tushdi.');
        } catch (e) {
          toast(`Restart xatosi: ${humanError(e)}`, 'err', { ttlMs: 7000 });
          logLine(`Restart xatosi: ${humanError(e)}`);
        }
      });
    };
  }
  ...
  $('btnInvStart').onclick = async () => {
    invDesired = true;
    applyStatus({ desiredInventory: true });
  ...
  $('btnInvStop').onclick = async () => {
    invDesired = false;
    applyStatus({ desiredInventory: false });
  ...
  $('btnWrite').onclick = async () => {
    await runBusy($('btnWrite'), 'Yozilmoqda…', async () => {
      try {
        ...
        const out = formatWriteOutput(result, body);
        setText('rwOut', out);
        const rc = normalizeRc(result);
        if (rc !== undefined && rc !== 0) {
          const desc = describeReturnCode(rc);
          toast(`Yozish xatosi: rc=${rc}${desc ? ` (${desc})` : ''}`, 'err', { ttlMs: 7000 });
          logLine(`Yozish xatosi: rc=${rc}${desc ? ` (${desc})` : ''}`);
          return;
        }
  ...
  if ($('btnRwEpcWrite')) {
    $('btnRwEpcWrite').onclick = async () => {
      await runBusy($('btnRwEpcWrite'), 'Yozilmoqda…', async () => {
        try {
          await maybeStopInventoryForRw();
          const body = buildSimpleEpcWriteBody();
          const same = body.epc === body.data;
          const ok = window.confirm(
            `EPC o‘zgartirishni tasdiqlaysizmi?\n\nEski: ${body.epc}\nYangi: ${body.data}\n\n` +
              `Eslatma: faqat bitta tag yaqin tursin.${same ? '\nDiqqat: Eski va yangi EPC bir xil.' : ''}`,
          );
          if (!ok) {
            toast('EPC o‘zgartirish bekor qilindi.', 'warn');
            logLine('EPC o‘zgartirish bekor qilindi.');
            return;
          }

          const result = await api('/api/write', body);
          let out = formatWriteOutput(result, body);
          const rc = normalizeRc(result);
          if (rc !== undefined && rc !== 0) {
            const desc = describeReturnCode(rc);
            setText('rwOut', out);
            toast(`EPC o‘zgartirish xatosi: rc=${rc}${desc ? ` (${desc})` : ''}`, 'err', { ttlMs: 7000 });
            logLine(`EPC o‘zgartirish xatosi: rc=${rc}${desc ? ` (${desc})` : ''}`);
            return;
          }

          const verify = await verifyEpcWrite(body);
          if (verify.error) {
            out += `\n\nTekshiruv: xato — ${verify.error}`;
          } else {
            out += `\n\nTekshiruv: ${verify.ok ? 'OK (yangi EPC topildi)' : 'Topilmadi (tag eski EPC bilan qolgan bo‘lishi mumkin)'}`;
            if (verify.hex) out += `\nRead: ${groupHex(verify.hex)}`;
          }
          setText('rwOut', out);
          if (!verify.ok) {
            toast('EPC tekshiruvdan o‘tmadi. Tagni ajrating yoki parol/lockni tekshiring.', 'warn', {
              ttlMs: 8000,
            });
            logLine('EPC tekshiruvdan o‘tmadi.');
            return;
          }
          selectedEpc = body.data;
          if ($('rwEpc')) $('rwEpc').value = body.data;
          if ($('rwNewEpc')) $('rwNewEpc').value = body.data;
          if (body.epc && body.epc !== body.data) {
            tags.delete(body.epc);
            setText('statUnique', String(tags.size));
          }
          updateRwMeta();
          renderTags();
          scheduleTagViewRender();
          toast('EPC yangilandi. Agar boshqa tag eski EPC bilan qolsa, u baribir ko‘rinadi.', 'ok');
          logLine(`EPC yangilandi: ${body.epc} -> ${body.data}`);
        } catch (e) {
          const msg = humanError(e);
          setText('rwOut', msg);
          if (msg.includes('EPC')) flashField('rwEpc');
          if (msg.includes('Yangi EPC')) flashField('rwNewEpc');
          if (msg.toLowerCase().includes('parol')) flashField('rwPwd');
          toast(`EPC o‘zgartirish xatosi: ${msg}`, 'err', { ttlMs: 7000 });
          logLine(`EPC o‘zgartirish xatosi: ${msg}`);
        } finally {
          updateRwMeta();
        }
      });
  ...
  $('btnSetPower').onclick = async () => {
    await runBusy($('btnSetPower'), 'Sozlanmoqda…', async () => {
      try {
        ...
        powerChangeCooldownUntil = Date.now() + 800;
        ...
        toast(wasRunning ? 'Quvvat yangilandi. Skan to‘xtatildi — qayta boshlang.' : 'Quvvat yangilandi.', 'ok');
  ...
  if ($('btnAntPowerFill')) {
    $('btnAntPowerFill').onclick = () => {
      const raw = $('antPowerAll')?.value || $('power')?.value;
      const v = clampPowerValue(raw, { min: 0, max: 33 });
      if (v === null) {
        toast('Antenna quvvati noto‘g‘ri (0–33).', 'warn', { ttlMs: 5000 });
        return;
      }
      const count = getAntennaCountFromUi();
      applyAntPowerValues(new Array(count).fill(v));
      toast('Antenna quvvatlari to‘ldirildi.', 'ok');
      logLine('Antenna quvvatlari to‘ldirildi.');
    };
  }

  if ($('btnAntPowerGet')) {
    $('btnAntPowerGet').onclick = async () => {
      await runBusy($('btnAntPowerGet'), 'O‘qilmoqda…', async () => {
        try {
          const result = await api('/api/settings/ant-power', { op: 'get' });
          const powers = Array.isArray(result?.powers) ? result.powers : null;
          if (!powers || !powers.length) throw new Error('Antenna quvvatlari topilmadi.');
          applyAntPowerValues(powers);
          setBasicOut({ action: 'GET_ANT_POWER', ...result });
          toast('Antenna quvvati o‘qildi.', 'ok');
          logLine('Antenna quvvati o‘qildi.');
        } catch (e) {
          toast(`Antenna quvvatini o‘qish xatosi: ${humanError(e)}`, 'err', { ttlMs: 7000 });
          logLine(`Antenna quvvatini o‘qish xatosi: ${humanError(e)}`);
        }
      });
    };
  }
  ...
  if ($('btnAntPowerSet')) {
    $('btnAntPowerSet').onclick = async () => {
      await runBusy($('btnAntPowerSet'), 'Sozlanmoqda…', async () => {
        try {
          const wasRunning = Boolean(currentStatus.inventoryStarted);
          if (wasRunning) {
            await api('/api/inventory/stop', {});
          ...
          const values = collectAntPowerValues();
          if (!values.length) throw new Error('Antenna quvvatlari bo‘sh.');
          const payload = values.join('|');
          const result = await api('/api/settings/ant-power', { op: 'set', powers: payload });
          powerChangeCooldownUntil = Date.now() + 800;
          setBasicOut({ action: 'SET_ANT_POWER', ...result });
          toast(wasRunning ? 'Antenna quvvati saqlandi. Skan to‘xtatildi — qayta boshlang.' : 'Antenna quvvati saqlandi.', 'ok');
          logLine('Antenna quvvati saqlandi.');
        } catch (e) {
          toast(`Antenna quvvatini sozlash xatosi: ${humanError(e)}`, 'err', { ttlMs: 7000 });
          logLine(`Antenna quvvatini sozlash xatosi: ${humanError(e)}`);
        }
      });
    };
  }
...
async function init() {
  ...
  try {
    ...
    if (!raw) {
      const countRaw = Number($('readerType')?.value ?? 16);
      const count = Number.isFinite(countRaw) ? Math.max(1, Math.min(31, Math.trunc(countRaw))) : 16;
      const mask = Math.pow(2, count) - 1;
      if ($('antennaMask')) $('antennaMask').value = String(mask);
    }
  } catch {
    // ignore
  }
