...
public final class BridgeMain {
  ...
  private Object dispatch(String cmd, Map<String, Object> args) throws Exception {
    ...
    if ("START_READ".equals(cmd)) {
      requireConnected();
      ensureCallback();
      if (inventoryStarted) return jsonObj("rc", Integer.valueOf(0), "already", Boolean.TRUE);
      int rc = intRet(invokeAny(reader, "StartRead"));
      if (rc != 0) {
        // Try once: stop any stray inventory session, then retry.
        safeInvoke(reader, "StopRead");
        try {
          Thread.sleep(200);
        } catch (Exception ignored) {
        }
        rc = intRet(invokeAny(reader, "StartRead"));
      }
      if (rc != 0) {
    ...
    if ("STOP_READ".equals(cmd)) {
      ...
      if (reader != null) {
        try {
          rc = intRet(invokeAny(reader, "StopRead"));
        } catch (Exception e) {
          safeInvoke(reader, "StopRead");
          rc = -1;
        }
      ...
      inventoryStarted = false;
      sendEvent("STATUS", jsonObj("inventoryStarted", Boolean.FALSE, "rc", Integer.valueOf(rc)));
      return jsonObj("ok", Boolean.TRUE, "rc", Integer.valueOf(rc));
    }
    ...
    if ("GET_ANT_POWER".equals(cmd)) {
      Object ret = invokeAny(reader, "GetRfPowerByAnt");
      if (!(ret instanceof byte[])) throw new Exception("GetRfPowerByAnt failed");
      byte[] raw = (byte[]) ret;
      int[] powers = new int[raw.length];
      int[] rawVals = new int[raw.length];
      for (int i = 0; i < raw.length; i++) {
        rawVals[i] = raw[i] & 0xFF;
        powers[i] = rawVals[i] & 0x7F;
      }
      return jsonObj("powers", powers, "raw", rawVals, "count", Integer.valueOf(raw.length));
    }

    if ("SET_ANT_POWER".equals(cmd)) {
      String powersRaw = str(args.get("powers"), "");
      int count = intv(args.get("count"), 0);
      byte[] payload = parsePowerBytes(powersRaw, count);
      if (payload.length == 0) throw new Exception("Antenna quvvati bo'sh");
      int rc = intRet(invokeAny(reader, "SetRfPowerByAnt", payload));
      return jsonObj("rc", Integer.valueOf(rc), "count", Integer.valueOf(payload.length));
    }
  ...
  private boolean sameConnectArgs(Map<String, Object> last, String mode, String addr, int value, int baud, int readerType) {
    ...
    String lastMode = str(last.get("mode"), "").trim().toLowerCase();
    if (!mode.equals(lastMode)) return false;
    if ("serial".equals(mode)) {
      String lastDevice = str(last.get("device"), "").trim();
      if (!addr.equals(lastDevice)) return false;
      int lastBaud = intv(last.get("baud"), 0);
      if (baud > 0 && lastBaud > 0 && lastBaud != baud) return false;
    } else {
      String lastIp = str(last.get("ip"), "").trim();
      int lastPort = intv(last.get("port"), 0);
      if (!addr.equals(lastIp)) return false;
      if (value > 0 && lastPort > 0 && lastPort != value) return false;
    }
    int lastReaderType = intv(last.get("readerType"), readerType);
    if (readerType > 0 && lastReaderType > 0 && lastReaderType != readerType) return false;
    return true;
  }
  ...
  private static byte[] parsePowerBytes(String raw, int count) throws Exception {
    String s = raw == null ? "" : raw.trim();
    if (s.isEmpty()) return new byte[0];
    String[] parts = s.split("[|;\\s]+");
    if (parts.length == 1 && count > 1) {
      int v = clampPowerValue(parsePowerToken(parts[0]));
      byte[] out = new byte[count];
      for (int i = 0; i < count; i++) out[i] = (byte) (v & 0x7F);
      return out;
    }
    int n = parts.length;
    if (count > 0 && n > count) n = count;
    byte[] out = new byte[n];
    for (int i = 0; i < n; i++) {
      int v = clampPowerValue(parsePowerToken(parts[i]));
      out[i] = (byte) (v & 0x7F);
    }
    return out;
  }

  private static int parsePowerToken(String raw) {
    String s = raw == null ? "" : raw.trim();
  ...
  private static int clampPowerValue(int v) {
    if (v < 0) return 0;
    if (v > 63) return 63;
    return v;
  }
